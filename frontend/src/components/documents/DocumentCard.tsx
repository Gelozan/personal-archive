...
function FileIcon({ mimeType, className = "w-10 h-10" }: { mimeType: string; className?: string }) {
  if (mimeType === "application/pdf") {
    return (
      <svg className={className + " text-red-400"} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.2}>
  ...
  if (mimeType.startsWith("image/")) {
    return (
      <svg className={className + " text-emerald-400"} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.2}>
        <path strokeLinecap="round" strokeLinejoin="round"
  ...
  if (mimeType.includes("word") || mimeType.includes("document")) {
    return (
      <svg className={className + " text-blue-400"} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.2}>
        <path strokeLinecap="round" strokeLinejoin="round"
  ...
  return (
    <svg className={className + " text-slate-400"} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.2}>
      <path strokeLinecap="round" strokeLinejoin="round"
        d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
    </svg>
  );
...
export default function DocumentCard({ document, onClick, onShare, onTrash, onDownload, onMove,
  ...
  return (
    ...
    <div
      ...
      onDragEnd={onDragEnd}
      {...(longPressHandlers ?? {})}
      className="group bg-white rounded-xl border border-slate-200 p-2.5 md:p-4 cursor-pointer
        hover:border-sky-300 hover:shadow-md hover:shadow-sky-50
        transition-all duration-150 flex flex-col gap-3"
    >
      {/* Иконка файла */}
      <div className="w-9 h-9 md:w-12 md:h-12 rounded-lg bg-slate-50 flex items-center justify-center
        group-hover:bg-sky-50 transition-colors">
        <FileIcon mimeType={document.mime_type} className="w-6 h-6 md:w-10 md:h-10" />
      </div>
